...
async def translate_to_en(callback_query: types.CallbackQuery):
    global conn
    cursor = conn.cursor()
    cursor.execute(f"SELECT UTEXT FROM PHRASE WHERE ID={callback_query.chat.id}")
    data = cursor.fetchone()

    await bot.answer_callback_query(callback_query.id)
    try:
        await SendMessage(callback_query.chat.id, TextBlob(str(data).replace('(','').replace(')','').replace("'",'').replace(",",'')).translate(to = 'en'))
    except:
        await SendMessage(callback_query.chat.id, "Ошибка :c")
...
async def translate_to_sp(callback_query: types.CallbackQuery):
    ...
    await bot.answer_callback_query(callback_query.id)
    try:
        await SendMessage(callback_query.chat.id, TextBlob(str(data).replace('(','').replace(')','').replace("'",'').replace(",",'')).translate(to = 'es'))
    except:
        await SendMessage(callback_query.chat.id, "Ошибка :c")
...
async def translate_to_ch(callback_query: types.CallbackQuery):
    ...
    await bot.answer_callback_query(callback_query.id)
    try:
        await SendMessage(callback_query.chat.id, TextBlob(str(data).replace('(','').replace(')','').replace("'",'').replace(",",'')).translate(to = 'fr'))
    except:
        await SendMessage(callback_query.chat.id, "Ошибка :c")
...
async def translate_to_ger(callback_query: types.CallbackQuery):
    ...
    await bot.answer_callback_query(callback_query.id)
    try:
        await SendMessage(callback_query.chat.id, TextBlob(str(data).replace('(','').replace(')','').replace("'",'').replace(",",'')).translate(to = 'de'))
    except:
        await SendMessage(callback_query.chat.id, "Ошибка :c")
...
async def translate_to_ger(callback_query: types.CallbackQuery):
    ...
    await bot.answer_callback_query(callback_query.id)
    try:
        await SendMessage(callback_query.chat.id, TextBlob(str(data).replace('(','').replace(')','').replace("'",'').replace(",",'')).translate(to = 'zh'))
    except:
        await SendMessage(callback_query.chat.id, "Ошибка :c")
...
async def translate_to_ger(callback_query: types.CallbackQuery):
    ...
    await bot.answer_callback_query(callback_query.id)
    try:
        await SendMessage(callback_query.chat.id, TextBlob(str(data).replace('(','').replace(')','').replace("'",'').replace(",",'')).translate(to = 'ja'))
    except:
        await SendMessage(callback_query.chat.id, "Ошибка :c")
...
async def translate_to_ger(callback_query: types.CallbackQuery):
    ...
    await bot.answer_callback_query(callback_query.id)
    try:
        await SendMessage(callback_query.chat.id, TextBlob(str(data).replace('(','').replace(')','').replace("'",'').replace(",",'')).translate(to = 'ru'))
    except:
        await SendMessage(callback_query.chat.id, "Ошибка :c")
